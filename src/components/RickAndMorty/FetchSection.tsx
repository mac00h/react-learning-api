...
import classes from "./FetchSection.module.css";

interface FetchSectionProps {
  setCharacterID: React.Dispatch<React.SetStateAction<number | undefined>>;
}

export const FetchSection: React.FC<FetchSectionProps> = ({
  setCharacterID,
}) => {
  ...
  const fetchCharacter = () => {
    setCharacterID(inputValue);
  };

  const fetchRandom = () => {
    let random = getRandomNumber();
    setInputValue(random);
    setCharacterID(random);
  };
  ...
  return (
    <section className={classes["fetch-section"]}>
      <input
        ...
        value={inputValue || ""}
      />
      <button className={classes["fetch-button"]} onClick={fetchCharacter}>
        Fetch
      </button>
      <button
        className={classes["fetch-button-random"]}
        style={{
          backgroundColor: hover ? "var(--blue0)" : "var(--gray)",
        }}
      ...
      >
        <svg
          stroke={hover ? "var(--gray)" : "var(--blue0)"}
          fill={hover ? "var(--gray)" : "var(--blue0)"}
          strokeWidth="0"
          viewBox="0 0 512 512"
          height="1em"
          width="1em"
...
const getRandomNumber = (): number => {
  ...
  let storage = localStorage.getItem("fetchedIDs");
  let fetchedIDs = storage !== null ? JSON.parse(storage) : [];

  if (fetchedIDs.includes(random)) {
    return getRandomNumber();
  }

  let arr = fetchedIDs;
  arr.push(random);
  localStorage.setItem("fetchedIDs", JSON.stringify(arr));
  return random;
};
